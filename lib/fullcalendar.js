...
var defaults = {
	...
	timeFormat: { // for event elements
		...
		list: 'h:mm{ - h:mm}', //list and table views
		listFull: 'h:mm M d yyyy{ - h:mm M d yyyy}', //list and table views for events that span multiple days
	...
	todoCols: ['handle', 'time', 'title', 'status', 'percent'],
...
function Calendar(element, options, eventSources) {
	...
	function renderView(inc) {
		if (elementVisible()) {
			...
			if (!currentView.start || inc || date < currentView.start || date >= currentView.end || currentView.name == 'multiWeek') {
...
function EventManager(options, _sources) {
	...
	t.dumpEventSources = dumpEventSources;
	...
	function addEventSource(source, background) {
		source = _addEventSource(source, background);
		if (source) {
			pendingSourceCnt++;
	...
	function _addEventSource(source, background) {
...
function AgendaView(element, calendar, viewName) {
	...
	function buildSkeleton() {
		...
		/*var allDayHeight = 250;
		
		xallDayTable = $("<table style='position: relative; top:0px; left: 0px;'> <tr> <td style='text-align:center; vertical-align:middle'> </td> <td style='width: 100%;'> </td> </tr> </table>")
		    .appendTo(slotLayer);
		
    xallDayTableHeader = xallDayTable.find('td').slice(0, 1);
    xallDayTableBody = xallDayTable.find('td').slice(1, 2);                                                                   
        		
    xallDayHeader = $("<div style='width: 60px;'>" + opt('allDayText') + "</div>")
				.appendTo(xallDayTableHeader); 		
    		
		xslotLayer =
			$("<div class='allDayWrapper' style='max-height: " + allDayHeight + "px;'/>")
				.appendTo(xallDayTableBody);				
				
		if (opt('allDaySlot')) {
		
			daySegmentContainer =
				$("<div style='position:absolute;z-index:8;top:0;left:0; overflow-x:hidden; overflow-y:auto; height: 100%; width: 100%'/>")
					.appendTo(xslotLayer);
			...
			s =           
			  "<div>" +
				...
				"</table>" +
				"</div>";
			allDayTable = $(s).appendTo(xslotLayer);*/
    ...
    if (opt('allDaySlot')) {
		...
		dayContent =
			$("<div style='position:relative;width:100%;overflow:hidden;min-height:37px'/>")
				.appendTo(dayScroller);				
	...
	function setHeight(height, dateChanged) {
		...
		slotScroller.height(bodyHeight - allDayHeight - 1 - (opt('allDaySlot') ? 4 : 0));	
...
function DayEventRenderer() {
	...
	function daySegHTML(segs) { // also sets seg.left and seg.outerWidth
		...
		for (i=0; i<segCnt; i++) {
			...
			//III
			/*if(RRR) {
				html += '<img class="fc-event-img" src="caldavzap/images/refresh.svg" alt="Recurring" title="Recurring">';
			}
			if(AAA) {
				html += '<img class="fc-event-img" src="caldavzap/images/alarm.svg" alt="Alarm" title="Alarm">';
			}*/
			if (seg.isEnd && isEventResizable(event)) {
				html +=
	...
	function renderEventTime(event, seg) {
		...
		if (!datestr && event.allDay) {
			if(dayDiff(eventEnd, event.start)) {  //spans multiple days
				timestr = opt('allDayText') + " " + formatDates(event.start, eventEnd, timeFormatFullAllDay);
			}
			else {
...
function TableEventRenderer() {
	...
	function renderSegs(segs, modifiedEventId) {
		...
		prevMonth = $('<tbody class="fc-list-header"><tr><td class="fc-list-header fc-month-nav ' + headerClass + '" colspan="' + tableCols.length + '">' + opt('buttonText', 'prevMonth') + '</td></tr></tbody>').appendTo(table);
		...
		nextMonth = $('<tbody class="fc-list-header"><tr><td class="fc-list-header fc-month-nav ' + headerClass + '" colspan="' + tableCols.length + '">' + opt('buttonText', 'nextMonth') + '</td></tr></tbody>').appendTo(table);
...
function TableView(element, calendar) {
	...
	function render(date, delta) {
		...
		if (!table) {
			...
			if(opt('showDatepicker')) {
				dateInfoNumberSpan.html(origDate.getDate());
	...
	function buildSkeleton(date) {
		...
		if(opt('showDatepicker')) {
			...
			dateInfoNumberSpan = $('<span>').appendTo(dateInfoNumber);
			dateInfoNumberSpan.html(date.getDate());
	...
	function updateToday() {
		...
		$(table).find('tbody.fc-list-header').each(function() {
	...
	function setHeight(height, dateChanged) {
		if(opt('showDatepicker')) {
			...
			dateInfoText.css('padding-bottom', datepickerHeight - datepicker.children().outerHeight() + 5); //+5 for paddings
			var textHeight = dateInfoText.outerHeight();
			dateInfoNumber.css({'height': datepickerHeight - textHeight,
									'font-size': 145 - textHeight});
			dateInfoNumberSpan.height(145 - textHeight);
		}
	...
	function setWidth(width) {
		...
		if(opt('showDatepicker')) {
			var halfOuterWidth = Math.floor((outerWidth)/ 2);
			var minDatepickerWidth = $(datepicker).children().width();
			var padding = 10;
			$(dateInfo).css('width', halfOuterWidth - padding);

			var dpLeft = halfOuterWidth;
			var dpWidth = halfOuterWidth - padding;
			if(dpLeft < minDatepickerWidth) {
				dpLeft -= (minDatepickerWidth - dpLeft);
			}
			$(datepicker).css({'left': dpLeft,
								'width': dpWidth});
		}
...
function TodoEventRenderer() {
	...
	function renderSegs(segs, modifiedEventId) {
		...
		for (j in segs) {
			...
			if(event.end && event.end.getTime() < addDays(cloneDate(t.start), 2, false).getTime()) {
			...
			for (var col, c=0; c < tableCols.length; c++) {
				...
				if (col == 'handle') {
					...
					s += "<td class='fc-event-percent'>" + (event.percent ? event.percent + '%' : '&nbsp;') + "</td>";
				}
				else {
					s += "<td class='fc-event-" + col + "'>" + (event[col] ? htmlEscape(event[col]) : '&nbsp;') + "</td>";
				}
...
function TodoView(element, calendar) {
	...
	function buildSkeleton(date) {
		...
		if(!opt('simpleFilters')) {
			ft = '<table class="fc-filter-table">' +
					'<tr>' +
